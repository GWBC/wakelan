<template>
  <MainPage>
    <template #header>
    </template>

    <template #main>
      <el-tabs v-model="cur_select" type="border-card" class="docker navigation relative">
        <el-tab-pane class="h-full" name="store" label="商店">
          <ApplicationStore />
        </el-tab-pane>
        <el-tab-pane class="h-full" name="app" label="应用">
          <Application />
        </el-tab-pane>
        <el-tab-pane class="h-full" name="image" label="更多">
          <el-tabs v-model="cur_sub_select" tab-position="left" class="docker-more navigation relative">
            <el-tab-pane class="h-full" name="image" label="镜像">
              <template #label>
                <div class="flex items-center gap-1">
                  <el-icon size="20">
                    <svg xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512">
                      <path
                        d="M256 176a80 80 0 1080 80 80.09 80.09 0 00-80-80zm0 112a32 32 0 1132-32 32 32 0 01-32 32z" />
                      <path
                        d="M414.39 97.61A224 224 0 1097.61 414.39 224 224 0 10414.39 97.61zM256 368a112 112 0 11112-112 112.12 112.12 0 01-112 112z" />
                    </svg>
                  </el-icon>
                  <span>镜像</span>
                </div>
              </template>
              <Images v-if="cur_sub_select == 'image'" :group="group" />
            </el-tab-pane>
            <el-tab-pane class="h-full" name="container" label="容器">
              <template #label>
                <div class="flex items-center gap-1">
                  <el-icon size="20">
                    <svg xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512">
                      <path
                        d="M440.9 136.3a4 4 0 000-6.91L288.16 40.65a64.14 64.14 0 00-64.33 0L71.12 129.39a4 4 0 000 6.91L254 243.88a4 4 0 004.06 0zM54 163.51a4 4 0 00-6 3.49v173.89a48 48 0 0023.84 41.39L234 479.51a4 4 0 006-3.46V274.3a4 4 0 00-2-3.46zM272 275v201a4 4 0 006 3.46l162.15-97.23A48 48 0 00464 340.89V167a4 4 0 00-6-3.45l-184 108a4 4 0 00-2 3.45z" />
                    </svg>
                  </el-icon>
                  <span>容器</span>
                </div>
              </template>
              <Containers v-if="cur_sub_select == 'container'" :group="group" />
            </el-tab-pane>
            <el-tab-pane class="h-full" name="network" label="网络">
              <template #label>
                <div class="flex items-center gap-1">
                  <el-icon size="20">
                    <svg xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512">
                      <path
                        d="M414.39 97.74A224 224 0 1097.61 414.52 224 224 0 10414.39 97.74zM64 256.13a191.63 191.63 0 016.7-50.31c7.34 15.8 18 29.45 25.25 45.66 9.37 20.84 34.53 15.06 45.64 33.32 9.86 16.21-.67 36.71 6.71 53.67 5.36 12.31 18 15 26.72 24 8.91 9.08 8.72 21.52 10.08 33.36a305.36 305.36 0 007.45 41.27c0 .1 0 .21.08.31C117.8 411.13 64 339.8 64 256.13zm192 192a193.12 193.12 0 01-32-2.68c.11-2.71.16-5.24.43-7 2.43-15.9 10.39-31.45 21.13-43.35 10.61-11.74 25.15-19.68 34.11-33 8.78-13 11.41-30.5 7.79-45.69-5.33-22.44-35.82-29.93-52.26-42.1-9.45-7-17.86-17.82-30.27-18.7-5.72-.4-10.51.83-16.18-.63-5.2-1.35-9.28-4.15-14.82-3.42-10.35 1.36-16.88 12.42-28 10.92-10.55-1.41-21.42-13.76-23.82-23.81-3.08-12.92 7.14-17.11 18.09-18.26 4.57-.48 9.7-1 14.09.68 5.78 2.14 8.51 7.8 13.7 10.66 9.73 5.34 11.7-3.19 10.21-11.83-2.23-12.94-4.83-18.21 6.71-27.12 8-6.14 14.84-10.58 13.56-21.61-.76-6.48-4.31-9.41-1-15.86 2.51-4.91 9.4-9.34 13.89-12.27 11.59-7.56 49.65-7 34.1-28.16-4.57-6.21-13-17.31-21-18.83-10-1.89-14.44 9.27-21.41 14.19-7.2 5.09-21.22 10.87-28.43 3-9.7-10.59 6.43-14.06 10-21.46 1.65-3.45 0-8.24-2.78-12.75q5.41-2.28 11-4.23a15.6 15.6 0 008 3c6.69.44 13-3.18 18.84 1.38 6.48 5 11.15 11.32 19.75 12.88 8.32 1.51 17.13-3.34 19.19-11.86 1.25-5.18 0-10.65-1.2-16a190.83 190.83 0 01105 32.21c-2-.76-4.39-.67-7.34.7-6.07 2.82-14.67 10-15.38 17.12-.81 8.08 11.11 9.22 16.77 9.22 8.5 0 17.11-3.8 14.37-13.62-1.19-4.26-2.81-8.69-5.42-11.37a193.27 193.27 0 0118 14.14c-.09.09-.18.17-.27.27-5.76 6-12.45 10.75-16.39 18.05-2.78 5.14-5.91 7.58-11.54 8.91-3.1.73-6.64 1-9.24 3.08-7.24 5.7-3.12 19.4 3.74 23.51 8.67 5.19 21.53 2.75 28.07-4.66 5.11-5.8 8.12-15.87 17.31-15.86a15.4 15.4 0 0110.82 4.41c3.8 3.94 3.05 7.62 3.86 12.54 1.43 8.74 9.14 4 13.83-.41a192.12 192.12 0 019.24 18.77c-5.16 7.43-9.26 15.53-21.67 6.87-7.43-5.19-12-12.72-21.33-15.06-8.15-2-16.5.08-24.55 1.47-9.15 1.59-20 2.29-26.94 9.22-6.71 6.68-10.26 15.62-17.4 22.33-13.81 13-19.64 27.19-10.7 45.57 8.6 17.67 26.59 27.26 46 26 19.07-1.27 38.88-12.33 38.33 15.38-.2 9.81 1.85 16.6 4.86 25.71 2.79 8.4 2.6 16.54 3.24 25.21a158 158 0 004.74 30.07A191.75 191.75 0 01256 448.13z" />
                    </svg>
                  </el-icon>
                  <span>网络</span>
                </div>
              </template>
              <Networks v-if="cur_sub_select == 'network'" :group="group" />
            </el-tab-pane>
            <el-tab-pane class="h-full" name="import" label="恢复">
              <template #label>
                <div class="flex items-center gap-1">
                  <el-icon size="20">
                    <svg xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512">
                      <path
                        d="M256 48C141.31 48 48 141.32 48 256c0 114.86 93.14 208 208 208 114.69 0 208-93.31 208-208 0-114.87-93.13-208-208-208zm0 313a94 94 0 010-188h4.21l-14.11-14.1a14 14 0 0119.8-19.8l40 40a14 14 0 010 19.8l-40 40a14 14 0 01-19.8-19.8l18-18c-2.38-.1-5.1-.1-8.1-.1a66 66 0 1066 66 14 14 0 0128 0 94.11 94.11 0 01-94 94z" />
                    </svg>
                  </el-icon>
                  <span>恢复</span>
                </div>
              </template>
              <Import v-if="cur_sub_select == 'import'" :group="group" />
            </el-tab-pane>
          </el-tabs>
        </el-tab-pane>
      </el-tabs>
    </template>
  </MainPage>
</template>

<script setup lang="ts">
import MainPage from '@/components/MainPage.vue'
import Images from '@/components/docker/Images.vue'
import Networks from '@/components/docker/Networks.vue'
import Containers from '@/components/docker/Containers.vue'
import Import from '@/components/docker/Import.vue'
import Application from '@/components/docker/Application.vue'
import ApplicationStore from '@/components/docker/ApplicationStore.vue'
import { onMounted, ref } from 'vue'

const group: string = 'api/docker/'

const cur_select = ref('app')
const cur_sub_select = ref('image')

onMounted(() => {
})

</script>

<style>
.docker .el-tabs__content {
  height: calc(100% - 30px);
}

.docker-more .el-tabs__content {
  height: 100%;
}

.docker-more .el-tabs__item {
  height: 60px;
}

.app_container {
  display: flex;
  flex-wrap: wrap;
  gap: 10px;
}

.app_card {
  width: 200px;
  height: 280px;
  flex-grow: 1;
}
</style>